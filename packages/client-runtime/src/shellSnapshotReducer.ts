...
export function applyShellStreamEvent(
  ...
  switch (event.kind) {
    ...
    case "scheduled-job-upserted": {
      const scheduledJobs = snapshot.scheduledJobs.some((job) => job.id === event.job.id)
        ? Arr.map(snapshot.scheduledJobs, (job) => (job.id === event.job.id ? event.job : job))
        : Arr.append(snapshot.scheduledJobs, event.job);
      return { ...snapshot, scheduledJobs, snapshotSequence: event.sequence };
    }
    case "scheduled-job-removed":
      return {
        ...snapshot,
        scheduledJobs: Arr.filter(snapshot.scheduledJobs, (job) => job.id !== event.jobId),
        snapshotSequence: event.sequence,
      };
