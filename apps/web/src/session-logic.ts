...
export function hasActiveRunningTurn(session: SessionActivityState | null): boolean {
...
export function isLatestTurnSettled(
  ...
  if (hasActiveRunningTurn(session)) return false;
  return true;
}
...
export function deriveWorkLogEntries(
  ...
  for (const activity of ordered) {
    if (latestTurnId && activity.turnId !== latestTurnId && !isThreadScopedErrorActivity(activity))
      continue;
...
function isThreadScopedErrorActivity(activity: OrchestrationThreadActivity): boolean {
  return activity.turnId === null && activity.tone === "error";
}
