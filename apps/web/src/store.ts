...
function queuedFollowUpsEqual(
  left: readonly QueuedThreadFollowUp[] | undefined,
  right: readonly QueuedThreadFollowUp[],
): boolean {
  if (left === right) return true;
  if (left === undefined || left.length !== right.length) return false;
  return left.every((followUp, index) => {
    const nextFollowUp = right[index];
    return (
...
function writeThreadShellState(
  ...
  const nextTurnState: ThreadTurnState = {
    ...
    queuedFollowUps: state.threadTurnStateById[nextThread.shell.id]?.queuedFollowUps ?? [],
  };
  ...
  if (!threadTurnStatesEqual(state.threadTurnStateById[nextThread.shell.id], nextTurnState)) {
    nextState = {
      ...nextState,
      threadTurnStateById: {
        ...nextState.threadTurnStateById,
