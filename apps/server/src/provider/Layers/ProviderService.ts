  ...
  const startSessionWithFallback = Effect.fn("startSessionWithFallback")(function* (input: {
    ...
    return yield* attempt(initialResumeCursor).pipe(
      Effect.catchCause((cause) => {
        ...
        if (
          input.adapter.provider !== ProviderDriverKind.make("claudeAgent") ||
  ...
  const restartClaudeSessionAfterStaleSendTurn = Effect.fn(
    ...
    if (bindingInstanceId === undefined) {
      return yield* toValidationError(
        ...
        `Cannot recover thread '${input.sendTurnInput.threadId}' because no provider instance binding is persisted.`,
      );
    }
    ...
    return yield* Effect.gen(function* () {
      ...
      const turn = yield* routed.adapter.sendTurn(input).pipe(
        Effect.catchCause((cause) => {
          const error = Cause.squash(cause);
          if (
            routed.adapter.provider !== ProviderDriverKind.make("claudeAgent") ||
            !isClaudeStaleResumeError(error)
          ) {
            return Effect.failCause(cause);
          }
