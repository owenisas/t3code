...
const make = Effect.gen(function* () {
  ...
  const ensureSessionForThread = Effect.fn("ensureSessionForThread")(function* (
    ...
    if (existingSessionThreadId) {
      ...
      const shouldRestartForModelSelectionChange =
        (currentInfo.driverKind === ProviderDriverKind.make("claudeAgent") ||
          currentInfo.driverKind === ProviderDriverKind.make("cursor")) &&
        requestedModelSelection !== undefined &&
  ...
  const processTurnSteerRequested = Effect.fn("processTurnSteerRequested")(function* (
    ...
    const capabilities = yield* providerService.getCapabilities(threadProviderInstanceId);
    ...
    return yield* appendProviderFailureActivity({
      ...
      kind: "provider.turn.start.failed",
      summary: "Provider turn steer failed",
      detail: `Provider '${threadProviderInstanceId}' does not support steering active turns.`,
      turnId: thread.latestTurn?.turnId ?? null,
      createdAt: event.payload.createdAt,
    });
  });
