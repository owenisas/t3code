...
type ProjectionScheduledJobDbRow = typeof ProjectionScheduledJobDbRowSchema.Type;
type ProjectionScheduledJobRunDbRow = typeof ProjectionScheduledJobRunDbRowSchema.Type;
...
function mapScheduledJobRows(
  scheduledJobRows: ReadonlyArray<ProjectionScheduledJobDbRow>,
  scheduledJobRunRows: ReadonlyArray<ProjectionScheduledJobRunDbRow>,
): ReadonlyArray<ScheduledJob> {
  const runsByJob = new Map<string, ScheduledJobRun[]>();
  for (const row of scheduledJobRunRows) {
    ...
    runsByJob.set(row.jobId, [...(runsByJob.get(row.jobId) ?? []), run]);
  }

  return scheduledJobRows.map((row) => {
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listThreadQueuedFollowUpRows = SqlSchema.findAll({
    Request: Schema.Void,
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listThreadQueuedFollowUpRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listThreadQueuedFollowUps:query",
                "ProjectionSnapshotQuery.getSnapshot:listThreadQueuedFollowUps:decodeRows",
              ),
            ),
          ),
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              for (const row of queuedFollowUpRows) {
                ...
                const threadFollowUps = queuedFollowUpsByThread.get(row.threadId) ?? [];
      ...
      .withTransaction(
        Effect.all([
          ...
          listScheduledJobRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listScheduledJobs:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listScheduledJobs:decodeRows",
              ),
            ),
          ),
          listScheduledJobRunRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listScheduledJobRuns:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listScheduledJobRuns:decodeRows",
              ),
            ),
          ),
          listThreadMessageRows(undefined).pipe(
          ...
          listThreadQueuedFollowUpRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadQueuedFollowUps:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadQueuedFollowUps:decodeRows",
              ),
            ),
          ),
          listThreadRows(undefined).pipe(
        ...
        Effect.flatMap(
          ([
            ...
            Effect.sync(() => {
              ...
              for (let index = 0; index < scheduledJobRows.length; index += 1) {
                const row = scheduledJobRows[index];
                if (!row) {
                  continue;
                }
                updatedAt = maxIso(updatedAt, row.updatedAt);
              }
              for (let index = 0; index < scheduledJobRunRows.length; index += 1) {
                const row = scheduledJobRunRows[index];
                if (!row) {
                  continue;
                }
                updatedAt = maxIso(updatedAt, row.startedAt);
              ...
              for (let index = 0; index < queuedFollowUpRows.length; index += 1) {
                const row = queuedFollowUpRows[index];
                if (!row) {
                  continue;
                }
                updatedAt = maxIso(updatedAt, row.queuedAt);
      ...
      .withTransaction(
        Effect.all([
          ...
          listScheduledJobRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getShellSnapshot:listScheduledJobs:query",
                "ProjectionSnapshotQuery.getShellSnapshot:listScheduledJobs:decodeRows",
              ),
            ),
          ),
          listScheduledJobRunRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getShellSnapshot:listScheduledJobRuns:query",
                "ProjectionSnapshotQuery.getShellSnapshot:listScheduledJobRuns:decodeRows",
              ),
            ),
          ),
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const snapshot = {
                ...
                scheduledJobs: mapScheduledJobRows(
                  scheduledJobRows.filter((row) => row.deletedAt === null),
                  scheduledJobRunRows,
                ),
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        starredAt: threadRow.value.starredAt,
